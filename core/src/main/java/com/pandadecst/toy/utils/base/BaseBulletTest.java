...
package com.pandadecst.toy.utils.base;
...
public class BaseBulletTest extends BulletTest {
	...
	public void show() {
        ...
        groundModel.calculateTransforms();
...
//                                    material);
		...
		world.addConstructor("ground", new BulletConstructor(groundModel, 0f, Bullet.obtainStaticNodeShape(groundModel.nodes))); // mass = 0: static body
	...
	public void render(float d) {
        cameraController.update(d);
		render(true);
        uistage.act();//Math.min(Gdx.graphics.getDeltaTime(),1/60f));
        uistage.draw();   
	}
